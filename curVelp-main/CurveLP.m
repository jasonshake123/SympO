...
%                          December 2015
%
%  Copyright (c) 2015 by Y. Yang.
%  All rights reserved.
%
% This function finds the optimal solution for standard linear programming
% problems using an arc-search interior-point method. It implements the
% Algorithm in [Yaguang Yang]. This algorithm is almost identical to the well-
% known Mehrotra's algorithm except that CurveLP.m searches the optimizer along 
% an arc while Mehrotra's algorithm searches the optimizer along a straight
% line [S. Wright]. To compare the two algorithms, two matlab functions,
% CurveLP.m and Mehrotra.m, are implemented using the the same pre-processor
% and the same post-process, starting from the same initial point, using the 
% same parameters, and terminating with the same stopping criterion as 
% described in [Yaguang Yang].
%
%
% Synopsis:
%            function [x,obj,kk,infe,m1,n1]=CurveLP(A,b,c,d) 
% Input arguments:
%   A: input matrix (real) with dimension nxn.
%   b: input vector (real) with dimension nx1.
%   c: input vector (real) with dimension nx1.
%   d: d=1 use a function to reduce A to full rank.
...
%
% References:
% [Yaguang Yang] Arc-Search Infeasible Interior-Point Algorithm for
% Linear Programming. arXiv:1406.4539 [math.OC] 2014.
...
%      shall be  communicated to  the authors.  The modified  routines 
%      will  remain the sole  property of the author.
%   2. Due  acknowledgment  must  be  made of the  use of this code in 
%      research  reports  or  publications.  A citation to author's 
%      paper [Yaguang Yang] should be made. Whenever such reports are 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%This version is a work version which includes recovering full solution vector x
%Parameter: d=1 for degenerate problems making matrix full rank 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Preprocess rule 2 4 6 8 10 are not implemented but keep in the code in case that some
%third party indepent checks my claim
if exist('d') == 0
    d = 0;
end
epsilon=0.00000001; 
...
if Aratio<10^6  % why did I put this?
     ...
     while i>=1
         ...
         if length(idxp)==1 
             aip=A(i,idxp);
             A_pst(x_indx(idxp))=1; b_post=[b_post;b(i)/aip]; % only one element in this row is positive
    ...
    for ii=length(x_tobe):-1:1
        ...
        for jj=1:length(idxp)
            if idxp(jj)~=x_tobe(ii)
                x_final(x_tobe(ii))=x_final(x_tobe(ii))-A_post(ii,idxp(jj))*x_final(idxp(jj));
            end
        end
    end
